...
def in_current_weather():
    ...
    def get_kwargs(path_names):
        ...
        for path_name in path_names:
            kwarg_dict = {
                "output_table": Table(
                    conn_id=gv.CONN_ID_DUCKDB, name=gv.IN_CURRENT_WEATHER_TABLE_NAME
                ),
    ...
    list_of_path_names = get_file_list(
        path=gv.CURRENT_WEATHER_DATA_FOLDER_PATH, conn_id="local_file_default"
    )
    ...
    import_weather_data = LoadFileOperator.partial(
        ...
        pool="duckdb",
        outlets=[gv.DS_DUCKDB_IN_CURRENT_WEATHER]
